            ...
            {!showAddForm ? (
              ...
              <form onSubmit={handleAdd} className="w-full max-w-4xl bg-zinc-900 p-8 rounded-2xl border-2 border-orange-500/30 grid grid-cols-1 md:grid-cols-4 gap-6 items-end animate-in fade-in zoom-in duration-300 shadow-2xl">
                <div className="md:col-span-2">
                  <label className="text-[10px] font-black text-zinc-500 uppercase mb-2 block tracking-widest">URL Slike</label>
                  <input
                    required
                    value={newImg.url}
                    onChange={e => setNewImg({ ...newImg, url: e.target.value })}
                    className="w-full bg-black border border-white/10 rounded-lg px-3 py-2 text-white text-xs outline-none focus:border-orange-500"
