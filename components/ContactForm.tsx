...
const ContactForm: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!serviceId || !templateId || !publicKey) {
      console.warn('EmailJS keys missing. Sending via mailto fallback.');
    ...
    import('@emailjs/browser').then((emailjs) => {
      emailjs.send(
        ...
        templateId,
        {
          ...
          to_email: 'pavlemaster2@gmail.com'
        },
        publicKey
      ).then(() => {
        setStatus('success');
            ...
            <div className="reveal stagger-1 mt-8 lg:mt-0">
              <form onSubmit={handleSubmit} className="space-y-3 lg:space-y-4 p-6 lg:p-8 bg-zinc-900/50 border border-white/10 shadow-2xl rounded-2xl text-left">
                ...
                <button
                  ...
                  className={`w-full py-3 lg:py-4 font-black uppercase tracking-[0.2em] transition-all relative overflow-hidden flex items-center justify-center rounded-lg border border-white/10 text-[10px] lg:text-xs ${status === 'success' ? 'bg-green-600 text-white' : 'bg-orange-600 hover:bg-orange-500 text-white active:scale-[0.98]'
                    }`}
                >
